...
bool IsKeywordSupported(const char* response, const char* keyword)
{
	...
	if(response == NULL || keyword == NULL)
		return false;
	int res_len = (int)strlen(response);
	int key_len = (int)strlen(keyword);
	if(res_len < key_len)
		return false;
	int pos = 0;
...
unsigned char* CharToUnsignedChar(const char *strIn)
{
	...
	length = (unsigned long)strlen(strIn);
...
void CSmtp::Send()
{
	...
	try{
		...
		if(!(rcpt_count = (unsigned int)Recipients.size()))
		...
		for(FileId=0;FileId<Attachments.size();FileId++)
		{
            ...
            EncodedFileName += base64_encode((unsigned char *) FileName.c_str(), (unsigned int)FileName.size());
            EncodedFileName += "?=";
	...
	try
	{
		...
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			if(IsKeywordSupported(RecvBuf, "LOGIN") == true)
			{
				...
				// send login:
				std::string encoded_login = base64_encode(reinterpret_cast<const unsigned char*>(m_sLogin.c_str()),(unsigned int)m_sLogin.size());
				...
				// send password:
				std::string encoded_password = base64_encode(reinterpret_cast<const unsigned char*>(m_sPassword.c_str()), (unsigned int)m_sPassword.size());
			...
			else if(IsKeywordSupported(RecvBuf, "PLAIN") == true)
			{
				...
				unsigned int length = (unsigned int)strlen(SendBuf);
			...
			else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
			{
				...
				int passwordLength = (unsigned int) m_sPassword.size();
				...
				md5pass1.update(ustrChallenge, (unsigned int)decoded_challenge.size());
				md5pass1.finalize();
				...
				encoded_challenge = base64_encode(reinterpret_cast<const unsigned char*>(decoded_challenge.c_str()), (unsigned int)decoded_challenge.size());
			...
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				int find = (int)decoded_challenge.find("nonce");
				if(find<0)
				...
				std::string realm;
				find = (int)decoded_challenge.find("realm");
				if(find>=0){
				...
				MD5 md5a1a;
				md5a1a.update(ustrUsername, (unsigned int)m_sLogin.size());
				md5a1a.update((unsigned char*)":", 1);
				md5a1a.update(ustrRealm, (unsigned int)realm.size());
				md5a1a.update((unsigned char*)":", 1);
				md5a1a.update(ustrPassword, (unsigned int)m_sPassword.size());
				md5a1a.finalize();
				...
				md5a1b.update(ustrNonce, (unsigned int)nonce.size());
				md5a1b.update((unsigned char*)":", 1);
				md5a1b.update(ustrCNonce, (unsigned int)strlen(cnonce));
				...
				md5a2.update(ustrUri, (unsigned int)uri.size());
				...
				md5.update(ustrNonce, (unsigned int)nonce.size());
				md5.update((unsigned char*)":", 1);
				md5.update(ustrNc, (unsigned int)strlen(nc));
				md5.update((unsigned char*)":", 1);
				md5.update(ustrCNonce, (unsigned int)strlen(cnonce));
				md5.update((unsigned char*)":", 1);
				md5.update(ustrQop, (unsigned int)qop.size());
				md5.update((unsigned char*)":", 1);
				md5.update(ua2, 32);
				md5.finalize();
				...
				encoded_challenge = base64_encode(ustrDigest, (unsigned int)strlen(SendBuf));
				delete[] ustrDigest;
				snprintf(SendBuf, BUFFER_SIZE, "%s\r\n", encoded_challenge.c_str());
				pEntry = FindCommandEntry(command_DIGESTMD5);
				SendData(pEntry);
...
{
	return (unsigned int)MsgBody.size();
}
...
void CSmtp::ReceiveData_SSL(SSL* ssl, Command_Entry* pEntry)
{
	...
	while(!bFinish)
	{
		...
		if((res = select((int)(hSocket+1), &fdread, &fdwrite, NULL, &time)) == SOCKET_ERROR)
		{
			FD_ZERO(&fdread);
			FD_ZERO(&fdwrite);
			throw ECSmtp(ECSmtp::WSA_SELECT);
...
{
	int offset = 0,res,nLeft = (int)strlen(SendBuf);
	fd_set fdwrite;
	fd_set fdread;
	timeval time;
