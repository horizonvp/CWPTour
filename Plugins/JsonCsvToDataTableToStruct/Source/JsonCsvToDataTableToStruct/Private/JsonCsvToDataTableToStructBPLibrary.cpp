...
bool UJsonCsvToDataTableToStructBPLibrary::Generic_StructToJsonObjectStringV2(FString& OutJsonString, const UStruct* StructDefinition, const void* Struct)
{
	CustomJsonConverter::UStructToJsonObjectString(StructDefinition, Struct, OutJsonString);
...
bool UJsonCsvToDataTableToStructBPLibrary::Generic_StructArrayToJsonObjectStringV2(FString& OutJsonString, FArrayProperty* StructDefinition, void* Struct)
{
	...
	{
		CustomJsonConverter::UStructToJsonObjectString(StructType, ArrayHelper.GetRawPtr(i), tempString);
		OutJsonString += tempString;

		if (i < ArrayHelper.Num() - 1) OutJsonString += ",\n\r";
	}
	OutJsonString += "\n\r]";
	return true;
}

bool UJsonCsvToDataTableToStructBPLibrary::JsonStringToStructV2(FString InJsonString, UStruct*& Struct)
{
	// We should never hit this!  stubs to avoid NoExport on the class.
	check(0);
	return false;
...
bool UJsonCsvToDataTableToStructBPLibrary::Generic_JsonStringToStructV2(FString InJsonString, UStruct* StructDefinition, void* OutPtr)
{
	...
	{
		UE_LOG(LogJson, Warning, TEXT("JsonStringToStructV2 - Unable to parse json=[%s]"), *InJsonString);
		return false;
	}
	if (!CustomJsonConverter::JsonObjectToUStruct(JsonObject.ToSharedRef(), StructDefinition, OutPtr))
	{
		UE_LOG(LogJson, Warning, TEXT("JsonStringToStructV2 - Unable to deserialize. json=[%s]"), *InJsonString);
		return false;
	}
	return true;
}

bool UJsonCsvToDataTableToStructBPLibrary::JsonStringToStructArrayV2(FString InJsonString, TArray<UScriptStruct*>& StructArray)
{
	check(0);
	return false;
}

bool UJsonCsvToDataTableToStructBPLibrary::Generic_JsonStringToStructArrayV2(FString InJsonString, FArrayProperty* StructDefinition, void* Struct)
{
	if (!StructDefinition || !Struct) return false;
	...
	{
		UE_LOG(LogJson, Warning, TEXT("JsonStringToStructArrayV2 - Unable to parse. json=[%s]"), *InJsonString);
		return false;
	}
	...
	for (int32 i = 0; i < JsonArray.Num(); ++i)
	{
		...
		{
			UE_LOG(LogJson, Warning, TEXT("JsonStringToStructArrayV2 - Array element [%i] was not an object."), i);
			return false;
		}
		if (!CustomJsonConverter::JsonObjectToUStruct(Value->AsObject().ToSharedRef(), StructType, &(*ArrayHelper.GetRawPtr(i))))
		{
			UE_LOG(LogJson, Warning, TEXT("JsonStringToStructArrayV2 - Unable to deserialize. json=[%s]"), *InJsonString);
			return false;
		}
	}
...
{
	if (!TableType) return nullptr;

	FString DTName = InDataTableName;

	if (InDataTableName.IsEmpty())
	{
		DTName = FGuid::NewGuid().ToString();
	}
...
#if !WITH_EDITOR
	TObjectPtr<UField> ptr = TableType->Children;
