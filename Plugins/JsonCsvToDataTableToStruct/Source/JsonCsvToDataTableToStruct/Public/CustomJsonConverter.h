...
class JSONCSVTODATATABLETOSTRUCT_API CustomJsonConverter
{
public:
...
public: // JSON -> UStruct
	...
	static bool JsonObjectStringToUStruct(const FString& JsonString, OutStructType* OutStruct, int64 CheckFlags = 0, int64 SkipFlags = 0)
	{
		...
		if (!CustomJsonConverter::JsonObjectToUStruct(JsonObject.ToSharedRef(), OutStruct, CheckFlags, SkipFlags))
		{
	...
	*/
	...
	static bool JsonArrayToUStruct(const TArray<TSharedPtr<FJsonValue>>& JsonArray, TArray<OutStructType>* OutStructArray, int64 CheckFlags = 0, int64 SkipFlags = 0)
	{
		...
		for (int32 i = 0; i < JsonArray.Num(); ++i)
		{
			...
			if (!CustomJsonConverter::JsonObjectToUStruct(Value->AsObject().ToSharedRef(), OutStructType::StaticStruct(), &(*OutStructArray)[i], CheckFlags, SkipFlags))
			{
